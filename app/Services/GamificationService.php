...
class GamificationService
{
    ...
    public function addCoins(User $user, int $amount, array $metadata): array
    {
        ...
        if ($amount <= 0) {
            throw new \InvalidArgumentException('Coin amount must be greater than 0.');
        }
    ...
    public function useCoins(User $user, int $amount, array $metadata): array
    {
        ...
        if ($user->total_coin < $amount) {
            throw new \InvalidArgumentException('User does not have enough coins.');
        }
    ...
    public function addExp(User $user, int $amount, array $metadata): array
    {
        ...
        if ($amount <= 0) {
            throw new \InvalidArgumentException('EXP amount must be greater than zero.');
        }
    ...
    public function createCheckin(User $user, array $payload): array
    {
        return DB::transaction(function () use ($user, $payload) {
            ...
            $hasCheckedInThisMonth = \App\Models\Checkin::where('user_id', $user->id)
            ...
            if ($hasCheckedInThisMonth) {
                throw new \InvalidArgumentException('You have already checked in at this place this month.');
            }
            ...
            $user->update([
                'total_coin' => ($user->total_coin ?? 0) + $coinsEarned,
                'total_exp' => ($user->total_exp ?? 0) + $expEarned,
                'total_checkin' => ($user->total_checkin ?? 0) + 1,
            ]);

            $place->update([
                'total_checkin' => ($place->total_checkin ?? 0) + 1,
            ]);
    ...
    public function createReview(User $user, array $payload): array
    {
        return DB::transaction(function () use ($user, $payload) {
            ...
            $hasReviewedInThisMonth = \App\Models\Review::where('user_id', $user->id)
                ->where('place_id', $payload['place_id'])
                ->whereBetween('created_at', [$startOfMonth, $endOfMonth])
                ->exists();
            ...
            if ($hasReviewedInThisMonth) {
                throw new \InvalidArgumentException('You have already reviewed this place this month.');
            }
            ...
            $reviewStats = \App\Models\Review::where('place_id', $payload['place_id'])
     ...
     * @return array
     */
    public function grantAchievement(User $user, int $achievement_id, array $additional_info): array
    {
        return DB::transaction(function () use ($user, $achievement_id, $additional_info) {
            ...
            if (!$achievement->status) {
                throw new \InvalidArgumentException('This achievement is currently not active.');
            }

            // Cek apakah pengguna sudah memiliki achievement ini
            $hasAchievement = \App\Models\UserAchievement::where('user_id', $user->id)
                ->where('achievement_id', $achievement_id)
                ->where('status', true)
                ->exists();

            if ($hasAchievement) {
                throw new \InvalidArgumentException('You already have this achievement.');
            }
     ...
     * @return array
     */
    public function completeChallenge(User $user, int $challenge_id, array $additional_info = []): array
    {
        return DB::transaction(function () use ($user, $challenge_id, $additional_info) {
            ...
            if (!$challenge->status) {
                throw new \InvalidArgumentException('This challenge is currently not active.');
            }

            // Cek apakah user sudah menyelesaikan challenge ini
            $hasCompleted = \App\Models\UserChallenge::where('user_id', $user->id)
                ->where('challenge_id', $challenge_id)
                ->where('status', true)
                ->exists();

            if ($hasCompleted) {
                throw new \InvalidArgumentException('You have already completed this challenge.');
            }
     ...
     * @return array
     */
    public function redeemReward(User $user, int $reward_id, array $additional_info = []): array
    {
        return DB::transaction(function () use ($user, $reward_id, $additional_info) {
            ...
            if (!$reward->status) {
                throw new \InvalidArgumentException('This reward is currently not active.');
            }
            ...
            $additional_info = array_merge(['redemption_code' => 'XYZ-' . time()], $additional_info);
     ...
     * @return array
     */
    public function getCoinTransactions(int $user_id, array $options = []): array
    {
        $page = $options['page'] ?? 1;
    ...
    public function getAchievements(int $user_id): array
    {
        $achievements = \App\Models\Achievement::where('status', true)
            ->with(['userAchievements' => function($query) use ($user_id) {
    ...
    public function getChallenges(int $user_id, array $options = []): array
    {
        ...
        $challenges = $query->with(['userChallenges' => function($query) use ($user_id) {
                $query->where('user_id', $user_id)->where('status', true);
            }])
    ...
    public function getRewards(array $options = []): array
    {
        ...
        $query = \App\Models\Reward::where('status', true)
            ->where('stock', '>', 0);
