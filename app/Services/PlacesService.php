...
class PlacesService
{
    ...
    public function getWithMultipleFilters(array $filters, int $perPage = 10): array
    {
        $query = Place::query();
        ...
        if (isset($filters['foodType']) && is_array($filters['foodType']) && count($filters['foodType']) > 0) {
            $query->where(function ($q) use ($filters) {
                foreach ($filters['foodType'] as $ft) {
                    $q->where(function ($qq) use ($ft) {
                        $qq->whereRaw("JSON_SEARCH(additional_info, 'one', ?, NULL, '$.food_type[*]') IS NOT NULL", [(string) $ft])
                            ->orWhereRaw("JSON_CONTAINS(JSON_EXTRACT(additional_info, '$.food_type'), JSON_OBJECT('key', ?))", [(string) $ft]);
                    });
                }
            });
        }

        if (isset($filters['placeValue']) && is_array($filters['placeValue']) && count($filters['placeValue']) > 0) {
            $query->where(function ($q) use ($filters) {
                foreach ($filters['placeValue'] as $pv) {
                    $q->where(function ($qq) use ($pv) {
                        $qq->whereRaw("JSON_SEARCH(additional_info, 'one', ?, NULL, '$.place_value[*]') IS NOT NULL", [(string) $pv])
                            ->orWhereRaw("JSON_CONTAINS(JSON_EXTRACT(additional_info, '$.place_value'), JSON_OBJECT('key', ?))", [(string) $pv]);
                    });
                }
            });
        }
        ...
        // right before  $places = $query->paginate …
        \Illuminate\Support\Facades\Log::debug('FULL SQL WITH EVERY SCOPE', [
            'sql'      => $query->toSql(),
            'bindings' => $query->getBindings(),
        ]);
        \Illuminate\Support\Facades\Log::debug('ROWS BEFORE PAGINATE', $query->get()->toArray());
        $places = $page ? $query->paginate($perPage, ['*'], 'page', $page) : $query->paginate($perPage);
        
        // Load reviews untuk setiap place
        $places->load(['reviews' => function ($query) {
    ...
    public function reviews(int $placeId, array $filters = [], int $perPage = 10, ?int $page = null): array
    {
        ...
        $reviews = $page ? $query->paginate($perPage, ['*'], 'page', (int) $page) : $query->paginate($perPage);

        $avg = (float) (\App\Models\Review::where('place_id', $placeId)->approved()->avg('rating') ?? 0);
        $total = (int) (\App\Models\Review::where('place_id', $placeId)->approved()->count());
        $hist = [];
        for ($i = 1; $i <= 5; $i++) {
            $hist[$i] = (int) (\App\Models\Review::where('place_id', $placeId)->approved()->where('rating', $i)->count());
        }
